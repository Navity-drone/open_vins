...
void StateHelper::EKFPropagation(
  ...
  if (order_NEW.empty() || order_OLD.empty()) {
    PRINT_ERROR(RED "StateHelper::EKFPropagation() - Called with empty "
                    "variable arrays!\n" RESET);
  ...
  for (size_t i = 0; i < order_NEW.size() - 1; i++) {
    if (order_NEW.at(i)->id() + order_NEW.at(i)->size() !=
        order_NEW.at(i + 1)->id()) {
      PRINT_ERROR(RED "StateHelper::EKFPropagation() - Called with "
                      "non-contiguous state elements!\n" RESET);
      PRINT_ERROR(
          RED "StateHelper::EKFPropagation() - This code only support a state "
...
void StateHelper::marginalize(std::shared_ptr<State> state,
  ...
  if (std::find(state->_variables.begin(), state->_variables.end(), marg) ==
    ...
    PRINT_ERROR(RED "StateHelper::marginalize() - Called on variable that is "
                    "not in the state\n" RESET);
    PRINT_ERROR(RED "StateHelper::marginalize() - Marginalization, does NOT "
                    "work on sub-variables yet...\n" RESET);
    std::exit(EXIT_FAILURE);
  }
...
StateHelper::clone(std::shared_ptr<State> state,
  ...
  if (new_clone == nullptr) {
    PRINT_ERROR(RED "StateHelper::clone() - Called on variable is not in the "
                    "state\n" RESET);
    PRINT_ERROR(RED "StateHelper::clone() - Ensure that the variable specified "
                    "is a variable, or sub-variable..\n" RESET);
    std::exit(EXIT_FAILURE);
  }
  ...
  for (int r = 0; r < R.rows(); r++) {
    for (int c = 0; c < R.cols(); c++) {
      if (r == c && R(0, 0) != R(r, c)) {
        ...
        PRINT_ERROR(RED "StateHelper::initialize() - Found a value of %.2f "
                        "verses value of %.2f\n" RESET,
                    R(r, c), R(0, 0));
        std::exit(EXIT_FAILURE);
        ...
        PRINT_ERROR(RED "StateHelper::initialize() - Found a value of %.2f at "
                        "row %d and column %d\n" RESET,
                    R(r, c), r, c);
        std::exit(EXIT_FAILURE);
      }
...
void StateHelper::initialize_invertible(
  ...
  for (int r = 0; r < R.rows(); r++) {
    for (int c = 0; c < R.cols(); c++) {
      if (r == c && R(0, 0) != R(r, c)) {
        PRINT_ERROR(RED "StateHelper::initialize_invertible() - Your noise is "
                        "not isotropic!\n" RESET);
        PRINT_ERROR(RED "StateHelper::initialize_invertible() - Found a value "
                        "of %.2f verses value of %.2f\n" RESET,
                    R(r, c), R(0, 0));
        std::exit(EXIT_FAILURE);
      } else if (r != c && R(r, c) != 0.0) {
        PRINT_ERROR(RED "StateHelper::initialize_invertible() - Your noise is "
...
void StateHelper::augment_clone(std::shared_ptr<State> state,
  ...
  if (state->_clones_IMU.find(state->_timestamp) != state->_clones_IMU.end()) {
    PRINT_ERROR(RED "TRIED TO INSERT A CLONE AT THE SAME TIME AS AN EXISTING "
                    "CLONE, EXITING!#!@#!@#\n" RESET);
    std::exit(EXIT_FAILURE);
  }
