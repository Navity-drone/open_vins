...
ROS2Visualizer::ROS2Visualizer(std::shared_ptr<rclcpp::Node> node,
  ...
  pub_odomimu =
      node->create_publisher<nav_msgs::msg::Odometry>("/ov_msckf/odomimu", 2);
  ...
  pub_posegt = node->create_publisher<geometry_msgs::msg::PoseStamped>(
      "/ov_msckf/posegt", 2);
  PRINT_DEBUG("Publishing: %s\n", pub_posegt->get_topic_name());
  pub_pathgt =
      node->create_publisher<nav_msgs::msg::Path>("/ov_msckf/pathgt", 2);
  PRINT_DEBUG("Publishing: %s\n", pub_pathgt->get_topic_name());

  // Loop closure publishers
  pub_loop_pose =
      node->create_publisher<nav_msgs::msg::Odometry>("/ov_msckf/loop_pose", 2);
  pub_loop_point = node->create_publisher<sensor_msgs::msg::PointCloud>(
      "/ov_msckf/loop_feats", 2);
  pub_loop_extrinsic = node->create_publisher<nav_msgs::msg::Odometry>(
      "/ov_msckf/loop_extrinsic", 2);
  pub_loop_intrinsics = node->create_publisher<sensor_msgs::msg::CameraInfo>(
      "/ov_msckf/loop_intrinsics", 2);
  ...
  if (save_total_state) {
    ...
    node->declare_parameter<std::string>("filepath_gt",
