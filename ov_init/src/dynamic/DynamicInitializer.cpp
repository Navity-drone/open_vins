...
bool DynamicInitializer::initialize(
  ...
  if (num_measurements < system_size) {
    PRINT_WARNING(YELLOW "[init-d]: not enough feature measurements (%d meas "
                         "vs %d state size)!\n" RESET,
  ...
  PRINT_DEBUG("[init-d]: system of %d measurement x %d states created (%d "
              "features, %s)\n",
  ...
  PRINT_INFO("[init-d]: %d iterations | %zu states, %zu feats (%zu valid) | %d "
             "param and %d res | cost %.4e => %.4e\n",
             (int)summary.iterations.size(), map_states.size(),
             map_features.size(), count_valid_features, summary.num_parameters,
