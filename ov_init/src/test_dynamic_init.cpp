  ...
  while (sim.ok()) {
    ...
    if (hascam) {
      ...
      if (success) {
        ...
        PRINT_INFO(REDPURPLE "e_ori = %.3f,%.3f,%.3f | %.3f,%.3f,%.3f,%.3f "
                             "(true) | %.3f,%.3f,%.3f,%.3f (est)\n" RESET,
        ...
        PRINT_INFO(REDPURPLE "nees total = %.3f | ori = %.3f | pos = %.3f "
                             "(ideal is 15 and 3)\n" RESET,
        ...
        assert(sim.get_state(oldestpose_time +
        ...
        for (auto const &_pose : _clones_IMU) {
          ...
          assert(sim.get_state(_pose.first +
                                   sim.get_true_parameters().calib_camimu_dt,
                               gt_imustate));
